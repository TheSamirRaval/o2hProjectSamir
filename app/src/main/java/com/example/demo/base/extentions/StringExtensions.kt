...
fun Spanned.constructSpannableString(
    spanClickListener: (spanUrl: String) -> Unit,
    @ColorInt spanColor: Int = Color.BLACK,
    spanBold: Boolean = false,
    spanUnderLine: Boolean = false
): CharSequence {
 ...
 */
fun CharSequence.makeClickSpannable(
    spanClickListener: (spanUrl: String) -> Unit,
    @ColorInt spanColor: Int = Color.BLACK,
    spanBold: Boolean = false,
    spanUnderLine: Boolean = false, args: String? = null, link: String? = null
): CharSequence {
    ...
    if (this.contains("[CDATA[")) {
        return this.toString().encodeToHtml()
            .constructSpannableString(spanClickListener, spanColor, spanBold, spanUnderLine)
    } else {
        val buffer = SpannableString(this)
        var argument = args
        if (argument == null) {
 ...
 */
fun CharSequence.makePrivacyPolicyClickSpannable(
    spanClickListener: (spanUrl: String) -> Unit,
    @ColorInt spanColor: Int = Color.BLACK,
    spanBold: Boolean = false,
    spanUnderLine: Boolean = false, text: String
): CharSequence {
    return this.makeClickSpannable(
        spanClickListener,
        spanColor,
        spanBold,
        spanUnderLine,
 ...
 */
fun CharSequence.makeTermsAndConditionClickSpannable(
    spanClickListener: (spanUrl: String) -> Unit,
    @ColorInt spanColor: Int = Color.BLACK,
    spanBold: Boolean = false,
    spanUnderLine: Boolean = false, text: String

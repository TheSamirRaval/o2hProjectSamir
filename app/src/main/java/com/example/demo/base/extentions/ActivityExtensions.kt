...
/**
 * This is our base class that will help developer to use existing func
 */
const val SHARED_ELEMENT_STATUS_BAR = "statusBar"
const val SHARED_ELEMENT_NAVIGATION_BAR = "navigationBar"

inline fun <reified T : Activity> Activity.startActivityWithAnimation(noinline extra: Intent.() -> Unit) {
...
fun Activity.startActivityForResultWithDefaultAnimations(intent: Intent, requestCode: Int) {
...
fun Activity.startActivityWithSharedElementTransition(intent: Intent, sharedElement: View) {
    ...
    val listOfSharedElements = mutableListOf<androidx.core.util.Pair<View, String>>()
    statusBar?.let {
        listOfSharedElements.add(
            androidx.core.util.Pair(
                it,
                SHARED_ELEMENT_STATUS_BAR
            )
        )
    }
    navBar?.let {
...
//        TAG("Try to enter full screen mode but failed")
    }
}
...
fun Window?.enterImmersiveMode() {
    ...
    if (Build.VERSION.SDK_INT >= 19) {
        visibility =
            visibility or View.SYSTEM_UI_FLAG_IMMERSIVE or View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY
    }
